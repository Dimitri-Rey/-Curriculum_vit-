...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Scroll doux sur les ancres commençant par #
    ...
    window.scrollTo({
    top: target.offsetTop - 50, // ajustez selon la hauteur du header
    behavior: "smooth",
    });
    // Si vous voulez refermer le menu burger après le clic sur un lien :
    navList && navList.classList.remove("show");
    }
    });
    });
    // Fade-in d’éléments : .fade-in
    ...
    if (rect.top <=window.innerHeight * 0.8) { fadeElements[i].classList.add("visible"); fadeElements.splice(i, 1); } } if (fadeElements.length===0) { window.removeEventListener("scroll", fadeInOnScroll); } } window.addEventListener("scroll", fadeInOnScroll); fadeInOnScroll(); }); /********************************************* * DEBOUNCE (pour optimiser certains scrolls) *********************************************/ function debounce(func, wait) { let timeout; return function executedFunction(...args) { const later=()=> {
    ...
    *********************************************/
    ...
    input.placeholder = "Captcha incorrect, réessayez";
    }
    });
    }
    });
